...
//  unsigned long current_t = micros();
  ...
  float temp = current_pos > 0? current_pos: current_pos* -1;
  if(abs(current_pos)>= 1.0472){
  
        if ( current_pos< 0  && current_pos > desired_pos || current_pos > 0 && desired_pos > current_pos){
              // checks if our current position is at or greater than  60 degrees
                    current_vel = 0;
                    pos_error = 0;
                    sp_error = 0;
            }
           else {
               current_vel = (((((x-x_offset_W)-oldPosition_W)*0.02197*1000000*M_PI/180.0)/(current_t-previous_t)));   //Angular Speed(rad/s)    <---- does this need to be updated?
          }
